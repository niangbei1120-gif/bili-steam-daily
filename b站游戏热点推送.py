...
def fetch_recent_videos(max_pages=3):
    ...
    for p in range(1, max_pages + 1):
        ...
        time.sleep(1)
    # 时间衰减热度分：播放量 / 发布小时数^0.6
    # 刚发布就爆了的视频 > 累积了好几天的高播放量老视频
    now = time.time()
    def hotness(v):
        hours = max(1, (now - v["pub_ts"]) / 3600)
        return v["play"] / (hours ** 0.6)
    all_videos.sort(key=hotness, reverse=True)
    return all_videos
...
def classify_via_deepseek(videos):
    ...
    bili_items = [v for v in videos if v.get("source") == "B站" and v["play"] >= 500][:50]
...
**要求：**
1.  **严苛筛选**：只保留与 PC / Steam 游戏直接相关的内容，**果断丢弃**：纯手游、主机独占游戏、线下活动、与游戏无关的财经/社会新闻、泛科技新闻。同一款游戏的同一事件只保留描述最完整的一条，禁止重复入选。**desc 描述必须基于原始标题/描述概括，不得凭空编造"官方活动""大型展会"等原文中不存在的事件背景。**
2.  **来源均衡**：所有平台（B站、机核、游民星空、3DM、其乐Keylol、indienova、游戏葡萄）地位平等，不能只选B站。尽量从多个来源选取最优质的内容，避免单一平台垄断。
3.  **特别过滤**：以下内容一律丢弃，不得入选任何分类：
    *   电子竞技相关：职业赛事、电竞战队、选手动态、赛事赞助、电竞俱乐部、CAC/ESL/Major等电竞赛事资讯。
    *   其乐Keylol 来源中，纯打折促销、史低特卖、喜加一领取类内容。
    *   平台促销活动：周末特卖、季度大促、史低折扣、捆绑包特惠等以打折降价为核心的内容。
4.  **精准分类**：将所有符合条件的内容归入以下四类，按顺序输出，优先级依次降低。
    *   **新游速报**：Steam新游上线、测试、公布发售日、重大版本更新。最多3条。**优先选择本周新上线或刚公布的新游戏**，而非老游戏的日常更新。
    *   **热度飙升**：在玩家社群中正在快速传播的热门事件、出圈话题、争议、名场面、热梗。最多3条。
    *   **圈内大事**：对游戏行业有影响的产业新闻、发行商重大决策、核心玩家社区事件。最多3条。
    *   **平台活动**：Steam新品节、游戏节等官方展示活动（仅当**多个来源的内容同时提及该活动**时可入选，单独一条试玩视频不能算作"活动"）；Steam平台规则调整、审核政策变更、商店标签更新等影响发行的平台机制变化。**不包含任何促销折扣内容。** 最多2条，放在最后。**关键：判断依据是候选人列表中是否有多个关联该活动的报道/视频，而非把单个游戏试玩包装成"官方活动"。**
5.  **游戏多样性（重要）**：日报的目的是让读者了解游戏圈的"新鲜事"，而不是某款游戏的粉丝频道。请严格遵守：
    *   每款游戏每天最多入选 **1 条**（即使该游戏在多个分类中都有相关事件）。选择最有新闻价值的那条，其余的果断丢弃。
    *   对于长期霸榜的热门游戏（如幻兽帕鲁、黑神话悟空等），仅在当天有真正的重大新闻时（如大型更新、新DLC、破纪录事件）才入选。日常视频、直播切片、MOD推荐、小型社区讨论等低信息量内容一律丢弃。
    *   **优先覆盖新游戏、小体量独立游戏、首次出现在日报中的游戏**。一个有趣的冷门新游比幻兽帕鲁的第100条日常视频更有日报价值。
    *   整份日报中，来自同一款游戏的内容不超过总数的 20%（即最多 2 条）。
6.  **输出格式**：
